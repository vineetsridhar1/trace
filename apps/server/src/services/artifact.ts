...
export function validateType(type: string, manifest: ArtifactBundleManifest): void {
  ...
  if (type === "trace.image.v1" && manifest.files.length !== 1) {
    throw new ValidationError("Image artifacts require exactly one file");
  }
  if (
    type === "trace.video.v1" &&
  ...
  if (type === "trace.video.v1" && manifest.files.length !== 1) {
    throw new ValidationError("Video artifacts require exactly one file");
  }
}
...
export class ArtifactService {
      ...
      result = await prisma.$transaction(async (tx) => {
        const artifact = await tx.artifact.create({
          data: {
            ...
            approvalStatus: type === "trace.visual-plan.v1" ? "PENDING" : null,
          },
        });
  ...
  async listPageForSessionGroup(input: {
    organizationId: string;
    ...
    type?: string;
    key?: string;
  }): Promise<{ items: Artifact[]; hasMore: boolean }> {
      ...
      input.before && input.beforeId
        ? {
            OR: [
              ...
              { createdAt: input.before, id: { lt: input.beforeId } },
            ],
          }
        : {};
  ...
  async approve(input: {
    ...
    if (
      artifact.approvalStatus === "PROCESSING" &&
      (artifact.approvalAction !== input.action || artifact.approvalPromptDigest !== promptDigest)
    ) {
      throw new ValidationError("This plan is already being approved with different options");
    }
      ...
      where: {
        ...
        OR: [
          { approvalStatus: "PENDING" },
          {
            ...
            processingStartedAt: { lt: new Date(now.getTime() - APPROVAL_LEASE_MS) },
          },
        ],
      },
      data: {
        ...
        approvalPromptDigest: promptDigest,
    ...
    if (input.action === "KEEP_CONTEXT") {
      ...
      implementationSession = await sessionService.start({
        ...
        actorType: "user",
        clientSource: input.clientSource,
        clientMutationId: `artifact-approval:${artifact.id}:session`,
        startEventId: `artifact-approval:${artifact.id}:session`,
        tool: source.tool,
        model: source.model,
      ...
      await prisma.artifact.updateMany({
        ...
        data: { implementationSessionId: implementationSession.id },
      });
      if (source.agentStatus !== "stopped" && source.agentStatus !== "done") {
