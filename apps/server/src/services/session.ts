...
This is a Trace app session in its own isolated cloud runtime. Build a full-stack app, not a static artifact or patch to an existing user repo. Use the provided Next.js/Tailwind/shadcn-compatible starter as the source of truth. Add routes, API handlers, persistence, and UI in project files. You may install npm packages (pnpm is available) and use sudo to install any other OS packages you need. Redis and PostgreSQL are already running and ready to use — do NOT install, initialize, or reconfigure them, create roles, or edit pg_hba/auth. For Postgres, read the DATABASE_URL environment variable and pass it straight to your client (\`new Pool({ connectionString: process.env.DATABASE_URL })\`, Prisma \`url = env("DATABASE_URL")\`, etc.); it is a complete, credentialed TCP URL (\`postgresql://user:pass@localhost:5432/app\`) for a ready database named \`app\` — do not parse it, override the user, or switch to a Unix socket. Redis is at REDIS_URL / redis://localhost:6379. Keep credentials out of git. Preserve data-trace-source attributes when adding inspectable UI elements. IMPORTANT: the dev server is already started and managed for you on port 3000 (host 0.0.0.0) and hot-reloads your file changes — do NOT run \`pnpm dev\`/\`next dev\` or otherwise start your own server, the port is already taken and a second one will crash. Just edit files; if you need to verify, curl http://localhost:3000. Commit and push meaningful checkpoints to the configured managed origin when the app reaches a working state. Sharing the live app is a valid final outcome.
</system-instruction>`;
