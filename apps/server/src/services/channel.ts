...
export class ChannelService {
  ...
  async addMember(channelId: string, userId: string, actorType: ActorType, actorId: string) {
    await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      if (actorType !== "agent") {
        const actorMembership = await tx.orgMember.findUniqueOrThrow({
          where: {
            userId_organizationId: { userId: actorId, organizationId: channel.organizationId },
          },
          select: { role: true },
        });
        const actorChannelMembership = await tx.channelMember.findUnique({
          where: { channelId_userId: { channelId, userId: actorId } },
          select: { leftAt: true },
        });
        const actorCanAddMember =
          actorMembership.role === "admin" || actorChannelMembership?.leftAt === null;
        if (!actorCanAddMember) {
          throw new AuthorizationError("You do not have permission to add members to this channel");
        }
      }
