...
export const bridgeAccessTypeResolvers = {
  BridgeRuntime: {
    connected: (runtime: { instanceId: string; organizationId: string }) =>
      runtimeAccessService.isRuntimeConnected(runtime.instanceId, runtime.organizationId),
    registeredRepoIds: async (runtime: {
      instanceId: string;
      organizationId: string;
      metadata?: unknown;
    }) => {
      return runtimeAccessService.listRuntimeRegisteredRepoIds({
        runtimeInstanceId: runtime.instanceId,
        organizationId: runtime.organizationId,
        persistedMetadata: runtime.metadata,
      });
    },
  ...
  LinkedCheckoutStatus: {
    ...
    changedFilesTotalCount: (status: {
      changedFiles?: unknown[] | null;
      changedFilesTotalCount?: number | null;
    }) =>
      typeof status.changedFilesTotalCount === "number"
        ? status.changedFilesTotalCount
        : Array.isArray(status.changedFiles)
          ? status.changedFiles.length
          : 0,
    changedFilesTruncated: (status: { changedFilesTruncated?: boolean | null }) =>
      status.changedFilesTruncated ?? false,
