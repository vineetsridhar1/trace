...
export function createUserLoader() {
  ...
  >(async (ids: readonly string[]) => {
    ...
    const userMap = new Map(
      users.map((u: { id: string; name: string | null; avatarUrl: string | null }) => [u.id, u]),
    );
...
export function createSessionLoader() {
  return createPrismaLoaderWithInclude(
    prisma.session.findMany.bind(prisma.session) as unknown as Parameters<
      typeof createPrismaLoaderWithInclude
    >[0],
    {
      createdBy: true,
      repo: true,
...
export function createSessionGroupLoader() {
  return createPrismaLoaderWithInclude(
    prisma.sessionGroup.findMany.bind(prisma.sessionGroup) as unknown as Parameters<
      typeof createPrismaLoaderWithInclude
    >[0],
    {
      channel: true,
...
export function createChannelMembershipLoader(userId: string) {
  return new DataLoader<string, boolean>(async (channelIds: readonly string[]) => {
...
export function createChatMembershipLoader(userId: string) {
  return new DataLoader<string, boolean>(async (chatIds: readonly string[]) => {
    const members = await prisma.chatMember.findMany({
