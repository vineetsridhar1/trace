  ...
  const previewTarget = useCallback(
    (offset: number) => {
      ...
      const target = Math.max(
        0,
        Math.min(ranking.length - 1, current.origin + Math.round(offset / ROW_STRIDE)),
      );
    ...
    <View style={styles.list}>
      {ranking.map((value, index) => (
        <RankingRow
          ...
          value={value}
          label={
            question.options.find((option) => (option.id ?? option.label) === value)?.label ?? value
          }
          index={index}
...
function RankingRow({
  ...
  index: number;
  count: number;
  drag: { value: string; origin: number; target: number } | null;
  onBegin: (value: string, index: number) => void;
  onPreview: (offset: number) => void;
  onFinish: (value: string, offset: number) => void;
  ...
  return (
    <GestureDetector gesture={gesture}>
      ...
      >
        <Text variant="subheadline" style={styles.number}>
          {index + 1}
        </Text>
        <Text variant="subheadline" style={styles.label}>
          {label}
        </Text>
        <Text variant="title2" style={styles.handle}>
          ≡
        </Text>
      </Animated.View>
    </GestureDetector>
