...
export function QuestionFlowFooter({ label, disabled, backVisible, onBack, onPrimary, bottomInset }: { label: string; disabled: boolean; backVisible: boolean; onBack: () => void; onPrimary: () => void; bottomInset: number }) {
  return (
    <View style={[styles.dock, { paddingBottom: Math.max(28, bottomInset) }]}>
      {backVisible ? <Pressable onPress={onBack} style={styles.back}><Text variant="subheadline" style={styles.backText}>Back</Text></Pressable> : null}
      <Glass preset="input" interactive={!disabled} tint="rgba(0,116,225,0.76)" style={[styles.glass, disabled && styles.disabled]}>
        <Pressable accessibilityRole="button" disabled={disabled} onPress={onPrimary} style={styles.action}>
...
const styles = StyleSheet.create({
  ...
  glass: { flex: 1, height: questionMetrics.actionHeight, borderRadius: 26, borderWidth: 1, borderColor: "rgba(255,255,255,0.26)" },
  disabled: { opacity: 0.45 }, action: { flex: 1, justifyContent: "center", paddingHorizontal: 20 }, label: { color: questionColors.foreground, fontWeight: "600" },
});
