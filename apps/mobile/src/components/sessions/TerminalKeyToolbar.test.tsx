...
vi.mock("@/theme", () => ({
  useTheme: () => ({ colors: { border: "#111", foreground: "#fff", surface: "#222", surfaceElevated: "#333" } }),
}));
...
describe("TerminalKeyToolbar", () => {
  ...
  it("forwards pasted text, terminal interrupt, and copy actions", async () => {
    ...
    await act(async () => {
      renderer = TestRenderer.create(<TerminalKeyToolbar disabled={false} onCopy={onCopy} onInput={onInput} />);
    });
