...
export function SpatialTabButton({
  ...
  const { attributes, listeners, setNodeRef: setDraggableNodeRef, isDragging } = useDraggable({ id: tab.id });
  ...
  return (
    <motion.div layout="position" layoutId={`spatial-tab-${tab.id}`} transition={tabRailSpring} className="shrink-0">
      <div
        ref={setNodeRef}
        className={cn(
      ...
      >
        ...
        >
          ...
          <span className={cn("truncate font-medium", compact ? "text-[10px]" : "text-xs")}>{tab.label}</span>
          {tab.status ? (
            <span className={cn(
