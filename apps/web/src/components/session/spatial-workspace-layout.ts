...
export type SpatialLayoutPreset = "single" | "columns" | "three-columns" | "four-columns" | "rows";
...
export function countSpatialColumnsInRow(node: SpatialNode, position: SpatialRowPosition): number {
  if (node.type !== "split" || node.direction !== "vertical") {
  ...
  return {
    ...
    root: mapGroups(layout.root, (group) => {
      ...
      return {
        ...group,
        tabIds: nextIds,
        activeTabId: group.activeTabId === sourceTabId ? replacementTabId : group.activeTabId,
      };
    }),
  };
}
...
export function dockSpatialTab(
  ...
  if (horizontal) {
    ...
    const groups =
      edge === "left" ? [newGroup, ...remainingGroups] : [...remainingGroups, newGroup];
  ...
  const nextRoot = createSplit(layout.nextSplitNumber, "vertical", children[0], children[1]);
  ...
  let root = mapGroups(layout.root, (group) => {
    if (group.id === sourceGroup.id) {
      ...
      return {
        ...
        activeTabId: resolveActiveTab(
          nextIds,
          group.activeTabId === tabId ? null : group.activeTabId,
        ),
      };
    }
...
function mapGroups(
  node: SpatialNode,
  mapper: (group: SpatialTabGroup) => SpatialTabGroup,
): SpatialNode {
...
function arrangeHorizontalRows(node: SpatialNode, focusedGroupId: string | null): SpatialNode {
  ...
  const focusedInRow = focusedGroupId ? groups.some((group) => group.id === focusedGroupId) : false;
  const weights = new Map(
    groups.map((group) => [
      group.id,
      focusedInRow ? (group.id === focusedGroupId ? 0.7 : 0.3 / Math.max(1, groups.length - 1)) : 1,
    ]),
  );
...
function getSpatialRow(root: SpatialNode, position: SpatialRowPosition): SpatialNode | null {
...
function createFlatSpatialSplit(
  ...
  for (let index = groups.length - 2; index >= 0; index -= 1) {
    root = createSplit(
      firstSplitNumber + groups.length - 2 - index,
      direction,
      groups[index],
      root,
    );
