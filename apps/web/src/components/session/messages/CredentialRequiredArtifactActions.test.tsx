...
async function renderActions(): Promise<ReactTestRenderer> {
  ...
  await act(async () => {
    renderer = create(
      <CredentialRequiredArtifactActions provider="anthropic" sessionId="session-1" />,
    );
  });
  return renderer;
}
...
describe("CredentialRequiredArtifactActions", () => {
  ...
  it("shows a retry failure after successfully storing the key", async () => {
    mutation.mockReturnValueOnce({ toPromise: vi.fn().mockResolvedValue({}) }).mockReturnValueOnce({
  ...
  it("saves a Codex access token and retries the blocked session", async () => {
    ...
    await act(async () => {
      renderer = create(
        <CredentialRequiredArtifactActions provider="openai" sessionId="session-1" />,
      );
    });
