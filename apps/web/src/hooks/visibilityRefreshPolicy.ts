...
export const DISCONNECTED_RESTART_DELAY_MS = 2_000;
...
export function getResumeAction(durationMs: number, connected: boolean): ResumeAction {
  if (durationMs <= HIDDEN_THRESHOLD_MS) return "none";
  if (durationMs <= SLEEP_RESUME_THRESHOLD_MS) return "refresh";
  return connected ? "refresh" : "refresh-and-restart";
}
