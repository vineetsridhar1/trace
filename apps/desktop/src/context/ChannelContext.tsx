...
export function ChannelProvider({ children }: { children: ReactNode }) {
  ...
  // Register this Electron instance with the server so the web app can discover it
  useEffect(() => {
    if (!activeServerId) return;
    const token = getAuthToken();
    if (!token) return;
    void window.traceAPI.setInstanceAuth(token, activeServerId);
    return () => {
      void window.traceAPI.clearInstanceAuth();
    };
  }, [activeServerId]);
