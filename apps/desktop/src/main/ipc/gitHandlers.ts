...
export function registerGitHandlers(): void {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        return await checkBranchesMerged(repoPath, targets, baseBranch);
      } catch (err) {
  ...
  ipcMain.handle(
    ...
    async (_event, repoPath: string, baseBranch: string) => {
      try {
        return await checkMainStatus(repoPath, baseBranch);
      } catch (err) {
  ...
  ipcMain.handle(
    ...
    async (_event, repoPath: string, baseBranch: string) => {
      try {
        return await pullMain(repoPath, baseBranch);
      } catch (err) {
        return { success: false, error: String(err) };
      }
    },
  ...
  ipcMain.handle(
    ...
    async (_event, repoPath: string, branchName: string, baseBranch: string, scopingDocsPath?: string, sourceBranch?: string) => {
      try {
        return await createGitBranch(repoPath, branchName, baseBranch, scopingDocsPath, sourceBranch);
      } catch (err) {
        return { success: false, error: String(err) };
      }
    },
...
async function listRepoBranches(repoPath: string) {
...
async function checkBranchesMerged(
  repoPath: string,
  targets: Array<{ workspaceId: string; branch: string }>,
  baseBranch: string,
) {
...
async function checkMainStatus(repoPath: string, baseBranch: string) {
...
async function pullMain(repoPath: string, baseBranch: string) {
  const fetchResult = await runProcess(
    "git",
    ["fetch", "origin", baseBranch],
    repoPath,
...
export function registerGitRelayActions(): void {
  registerRelayAction("listRepoBranches", async (params) => {
    const { repoPath } = params as { repoPath: string };
    return listRepoBranches(repoPath);
  });

  registerRelayAction("checkBranchesMerged", async (params) => {
    const { repoPath, targets, baseBranch } = params as {
      repoPath: string;
      targets: Array<{ workspaceId: string; branch: string }>;
      baseBranch: string;
    };
    return checkBranchesMerged(repoPath, targets, baseBranch);
  });

  registerRelayAction("checkMainStatus", async (params) => {
    const { repoPath, baseBranch } = params as {
      repoPath: string;
      baseBranch: string;
    };
    return checkMainStatus(repoPath, baseBranch);
  });

  registerRelayAction("pullMain", async (params) => {
    const { repoPath, baseBranch } = params as {
      repoPath: string;
      baseBranch: string;
    };
    return pullMain(repoPath, baseBranch);
  });

  registerRelayAction("createGitBranch", async (params) => {
    const { repoPath, branchName, baseBranch, scopingDocsPath, sourceBranch } =
      params as {
        repoPath: string;
        branchName: string;
        baseBranch: string;
    ...
    return createGitBranch(repoPath, branchName, baseBranch, scopingDocsPath, sourceBranch);
  });
}
