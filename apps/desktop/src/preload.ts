...
contextBridge.exposeInMainWorld("traceAPI", {
  ...
  instanceGetName: () =>
    ipcRenderer.invoke("instance:getName") as Promise<string>,

  instanceSetName: (name: string) =>
    ipcRenderer.invoke("instance:setName", name) as Promise<{
      success: boolean;
    }>,

  instanceSetPassword: (password: string | null) =>
    ipcRenderer.invoke("instance:setPassword", password) as Promise<{
      success: boolean;
      error?: string;
    }>,

  setInstanceAuth: (token: string, serverId: string) =>
    ipcRenderer.invoke("set-instance-auth", token, serverId) as Promise<void>,
