...
function fuzzyScore(target: string, query: string): number {
  ...
  for (let ti = 0; ti < lower.length && qi < q.length; ti++) {
    if (lower[ti] === q[qi]) {
      ...
      if (ti === 0 || lower[ti - 1] === '/' || lower[ti - 1] === '.' || lower[ti - 1] === '-' || lower[ti - 1] === '_') {
        score += 10;
      }
      score += 1;
      lastMatchIndex = ti;
...
// Module-level cache so both MessagePanel and ThreadPanel share the same file list
const fileCache = new Map<string, FileItem[]>();
const fetchPromises = new Map<string, Promise<FileItem[]>>();

function getRepoFiles(repoPath: string): Promise<FileItem[]> {
  const cached = fileCache.get(repoPath);
  if (cached) return Promise.resolve(cached);

  let promise = fetchPromises.get(repoPath);
  if (!promise) {
    promise = window.traceAPI.listRepoFiles(repoPath).then((result) => {
      fetchPromises.delete(repoPath);
      const items = result.success
        ? result.files.map((p) => ({ path: p }))
        : [];
...
export function useFileMention(
  ...
  useEffect(() => {
    ...
    let stale = false;
    getRepoFiles(repoPath).then((items) => {
      if (!stale) setFiles(items);
    });
    return () => { stale = true; };
  }, [repoPath]);
  ...
  const isOpen = mention !== null && !dismissed && (filteredFiles.length > 0 || mention.query.length > 0);
  ...
  // No-op: mentions are now derived from text, cleared when input is cleared
