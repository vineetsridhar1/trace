...
## Trace Design workspace adapter

This copy is embedded in a Trace Design workspace. The Trace artifact contract in `AGENTS.md` and `docs/ai-guidance.md` has precedence over this skill and every linked reference. Apply the full design judgment, command playbooks, craft floor, critique methods, and detector, with these translations:

- `design.brief.json` is the product and surface brief. Its `mode` is the canonical Persuade, Operate, Read, or Experience mode.
- `design-system/DESIGN.md` and the rest of `design-system/` are the selected, immutable design-system authority. `trace.tokens.json` records the session's executable direction. Do not create root `PRODUCT.md` or `DESIGN.md` files.
- `design.canvas.json` and `src/design/` are the artifact. Produce screens, flows, variations, and states on the existing canvas; never turn them into a routed production app.
- Do not run `context.mjs` in this workspace. Read the Trace files above instead, then load the one command reference that owns the request and load `reference/craft-floor.md` immediately before editing.
- Map `init` and `shape` into resolving `design.brief.json`; map `document` and `extract` into consuming the selected design system without modifying it.
- The Trace canvas already provides live HMR and review. Map `live` into creating or refining labeled variation artboards; do not run Impeccable live injection or modify `src/App.tsx`, `src/canvas/`, Vite, or the export runtime.
- Do not install or update Impeccable, manage hooks, pin commands, or run doctor from a generated Design workspace. The vendored detector runs through `pnpm design:check`.
- Do not let a reference's orchestration assumptions override the tools and permissions actually exposed by the session. Trace's required `pnpm design:check`, `pnpm design:review`, and `pnpm test` quality gate remains mandatory.

When a command reference conflicts with this adapter, preserve its design intent and use the Trace-native mechanism stated here.
