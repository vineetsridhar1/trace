...
- [ ] **F1** Cross-org IDOR: `session(id)` query
- [ ] **F2** Cross-org IDOR: `sessions(orgId)` query trusts caller-supplied orgId
- [ ] **F3** Cross-org IDOR: `ticket(id)` query
- [ ] **F4** Cross-org IDOR: `updateScopeAiMode` mutates by bare ID
...
- [ ] **F10** `ticketEvents` subscription has no auth
- [ ] **F11** `sessionPortsChanged` / `sessionStatusChanged` have no auth
- [ ] **F12** `sessionEvents` checks only org, not session access
- [ ] **F13** `runSession` resolver drops ctx (`_ctx`)
- [ ] **F14** Cloud session terminals attachable by any org member
...
- [ ] **F18** `orgEvents` membership cache bypasses non-message channel events
- [ ] **F19** Server binds to `0.0.0.0` unconditionally
...
- [ ] **F25** Upload MIME allows SVG; no `Content-Disposition: attachment`
- [ ] **F26** No key-version prefix on encrypted API tokens
- [ ] **F27** Hardcoded super-admin email
- [ ] **F28** (folded into F20)
...
- [ ] **F34** Upload key path check uses `includes("..")`, not normalization
