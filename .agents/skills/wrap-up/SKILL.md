...
- **Auto memory** (Codex writes for itself) — Debugging insights, patterns
  discovered during the session, project quirks. Tell Codex to save these:
  "remember that..." or "save to memory that..."
- **AGENTS.md** (instructions for Codex) — Permanent project rules,
  conventions, commands, architecture decisions that should guide all future
  sessions
- **`.Codex/rules/`** (modular project rules) — Topic-specific instructions
...
- **`Codex.local.md`** (private per-project notes) — Personal WIP context,
...
- Is it a permanent project convention? → AGENTS.md or `.Codex/rules/`
- Is it scoped to specific file types? → `.Codex/rules/` with `paths:`
  frontmatter
- Is it a pattern or insight Codex discovered? → Auto memory
- Is it personal/ephemeral context? → `Codex.local.md`
...
- **Skill gap** — Things Codex struggled with, got wrong, or needed multiple
  attempts
- **Friction** — Repeated manual steps, things user had to ask for explicitly
  that should have been automatic
- **Knowledge** — Facts about projects, preferences, or setup that Codex
