...
function isFinalSessionOutput(event: Event): boolean {
  if (event.eventType !== "session_output") return false;
  const payload = asJsonObject(event.payload);
  return payload?.type === "assistant" || payload?.type === "result" || payload?.type === "error";
}
...
export function upsertFetchedSessionEventsWithOptimisticResolution(
  ...
  const scopeKey = eventScopeKey("session", sessionId);
  if (events.some(isFinalSessionOutput)) {
