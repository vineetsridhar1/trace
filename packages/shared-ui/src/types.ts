...
export interface DiffComponentProps {
  ...
  diffType?: string;
  hunks: ParsedHunk[];
  children?: (hunks: ParsedHunk[]) => React.ReactNode;
  [key: string]: unknown;
}
...
export interface DiffRuntime {
  Diff: React.ComponentType<DiffComponentProps>;
  Hunk: React.ComponentType<HunkComponentProps>;
...
// Server URL configuration — apps call setServerUrl() at startup
let cachedServerUrl = "http://localhost:3100";
