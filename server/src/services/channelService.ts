...
export async function createChannel(data: { name: string; localRepoPath?: string | null; baseBranch?: string | null; githubUrl?: string | null }) {
  return prisma.channel.create({ data });
}

export async function updateChannel(id: string, data: { name?: string; localRepoPath?: string | null; baseBranch?: string | null; githubUrl?: string | null; creationScript?: string | null }) {
