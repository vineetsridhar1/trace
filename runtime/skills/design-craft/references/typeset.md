...
- **Platform-specific work:** follow the active workspace's platform and accessibility guidance.

If typography replacement would create a new identity, route through [new-work.md](new-work.md) and update the durable design-system guidance when the workspace owns one. Otherwise preserve confirmed families and improve their use.

## Two isolated assessments

Run these as separate passes. Complete the typographic assessment before reading mechanical results so automated findings do not anchor design judgment.
...
2. **Mechanical scan:** use the workspace's native checks to inspect contrast, overflow, arbitrary font values, missing assets, and implementation drift. Synthesize both assessments before editing, noting what each caught alone. A clean scan is a floor, not proof of good typography.
...
When the hierarchy holds, use [polish.md](polish.md) for the final path-wide pass.
