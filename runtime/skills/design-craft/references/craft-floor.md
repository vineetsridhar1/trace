...
- **Browser surfaces:** the parts you did not draw still carry the design. Text selection, the caret, focus rings, underline offset, tabular numerals, and—when appropriate—native scrollbar colors all ship with browser defaults that may not fit the design system. Theme visible details from the palette without replacing platform behavior or familiar affordances.
