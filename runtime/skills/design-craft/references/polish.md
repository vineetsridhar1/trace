...
An automated finding is defect evidence, not proof of quality. Inspect the rendered experience and real interaction path.

## 1. Establish the system

Read the available design-system guidance and representative tokens, shared components, patterns, and neighboring flows. If no formal system exists, use coherent project conventions.
...
If a prior critique or review report exists, use it as one input. Incorporate relevant P0/P1 findings and name the evidence read. Perform an independent pass either way.
...
- agreement with the available design-system guidance, neighboring features, and the user's scope.

Run the workspace's native checks and review workflow. Fix real defects and document only narrow intentional exceptions. A clean automated check does not replace visual judgment.
