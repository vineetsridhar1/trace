...
only when the selected channel has no repository. App conversions do not accept a channel or repo;
Trace creates an isolated managed repo and moves the session to cloud. Add `--tool`, `--model`, or
`--reasoning` when the destination needs an
