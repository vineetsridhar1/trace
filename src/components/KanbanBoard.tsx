    ...
    return (
      <div className="flex min-h-0 flex-1 gap-3 overflow-x-auto bg-[#1a1b26] px-3 py-3">
        {Array.from({ length: 4 }, (_, colIdx) => (
          <div key={colIdx} className="flex w-[280px] flex-shrink-0 flex-col gap-2 rounded-lg bg-[#1f2335] p-3">
            <div className="h-5 w-24 rounded bg-[#292e42] animate-pulse" />
            {Array.from({ length: colIdx === 0 ? 3 : 2 }, (_, cardIdx) => (
              <div key={cardIdx} className="flex flex-col gap-2 rounded-lg bg-[#16161e] p-3">
                <div className="h-4 w-4/5 rounded bg-[#292e42] animate-pulse" />
                <div className="h-3 w-3/5 rounded bg-[#292e42] animate-pulse" />
              </div>
            ))}
          </div>
        ))}
