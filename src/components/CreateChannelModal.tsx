...
export function CreateChannelModal({ serverId, onClose, onCreated, onLocalConfigSave }: CreateChannelModalProps) {
  ...
  const [executeSuggestScripts] = useSuggestScriptsLazyQuery();
  ...
  const handleSuggestScripts = useCallback(async () => {
    ...
    setSuggesting(true);
    try {
      const { data } = await executeSuggestScripts({
        variables: { localRepoPath },
      });
      if (data?.suggestScripts) {
        if (data.suggestScripts.setupScript) {
          setDefaultSetupScript(data.suggestScripts.setupScript);
        }
        if (data.suggestScripts.runScript) {
          setDefaultRunScript(data.suggestScripts.runScript);
        }
      }
    } catch {
  ...
  }, [localRepoPath, executeSuggestScripts]);
