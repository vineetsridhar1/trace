...
function injectHooks(targetDir: string) {
  ...
  const curlCmd =
    'curl -s -X POST http://localhost:3100/events -H "Content-Type: application/json" -d "$(cat)"';
...
function spawnPty(targetDir: string) {
  ...
  ptyProcess = pty.spawn(shell, ['-l', '-c', 'claude --dangerously-skip-permissions; exec $SHELL -l'], {
    name: 'xterm-256color',
    cols: 80,
    rows: 24,
    cwd: targetDir,
