---
name: publish-private-desktop-app
description: Build and publish the private Trace macOS desktop app release to GitHub
user_invocable: true
argument: Optional semver version, with or without a leading v
---

# Publish Private Desktop App

Publish the private Trace desktop app to the private GitHub releases feed.

## Target

- Production URL: `https://gettrace.org`
- Update repo: `opendoor-labs/trace`
- Git remote: `private`
...
1. **Confirm prerequisites**
   - Run `gh auth status` and verify the active account can write to `opendoor-labs/trace`.
...
2. **Choose the version**
   ...
   - If no argument was provided, inspect latest releases with:
     ...
     ```bash
     gh release list --repo opendoor-labs/trace --limit 10
     ```
   ...
   - Ensure the resulting tag does not exist:
     ...
     ```bash
     gh release view "v<VERSION>" --repo opendoor-labs/trace
     ```
...
5. **Dry-run the exact release**
   - Run from the repo root:
     ...
     TRACE_DESKTOP_UPDATE_REPO=opendoor-labs/trace \
