...
- **No Zustand selectors that return freshly-built objects/arrays.** A selector that maps/filters into new objects produces a new snapshot on every render — even wrapped in `useShallow` (it only compares one level deep, and the inner objects are new references) — which makes `useSyncExternalStore` loop until React throws "Maximum update depth exceeded". Subscribe to the stable raw table/value (e.g. `useEntityStore((s) => s.channels)`) and derive the mapped array in `useMemo`. `useShallow` is only safe when the selector returns primitives or stable references (e.g. arrays of string IDs).
