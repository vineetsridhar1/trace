...
5. **Message-row utils are duplicated across web and mobile.** `formatTime`, `formatCommandLabel`, `stripMatchingQuotes`, `truncate`, `serializeUnknown`, `getCommandPrefix`, and `stripPromptWrapping` live in both `apps/web/src/components/session/messages/utils.ts` (+ `interactionModes.tsx` + `CommandExecutionRow.tsx`) and `apps/mobile/src/components/sessions/nodes/utils.ts`. They're platform-free and belong in `@trace/client-core` per plan §7.1. Pair with the `SESSION_DETAIL_QUERY` extraction in follow-up 1.
6. **Markdown link scheme is unvalidated.** `Markdown.tsx`'s `onLinkPress` calls `Linking.openURL(url)` directly on every link. Assistant content is LLM-generated, so a crafted link could invoke `tel:`, `sms:`, or custom-scheme handlers. Harden by checking `Linking.canOpenURL` and/or whitelisting `http(s)` (plus `mailto`) before dispatching. Picked up most naturally in ticket 30 (polish).
