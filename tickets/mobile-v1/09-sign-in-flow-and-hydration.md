...
- **Org switcher sheet** (`apps/mobile/src/components/auth/OrgSwitcherSheet.tsx`, <200 lines):
  ...
  - On select: `setActiveOrg(id)`, tear down ambient subscription, re-hydrate, re-subscribe.
...
- [ ] Org switcher changes active org, rehydrates, resubscribes
- [ ] Sign-out clears state and returns to sign-in
- [ ] 401 from any query clears auth and returns to sign-in
- [ ] All files <200 lines
...
4. Switch org via settings → tab content reflects new org.
5. Sign out → back to sign-in; token removed from Keychain (verify via Xcode device console).
