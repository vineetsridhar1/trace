...
Wire up both the custom scheme (`trace://`) and Apple universal links (`https://trace.app/m/...`) so that tapping notifications, pasted links, and web-to-mobile transitions all land on the correct screen with correct state. Expo Router handles most of this once config is right.
...
- **Universal links (`https://trace.app/m/...`)**:
  - Add `ios.associatedDomains: ['applinks:trace.app']` to `app.json`.
  - Server: host `.well-known/apple-app-site-association` at `https://trace.app/` with:
    ```json
    {
      "applinks": {
        "apps": [],
...
- [ ] Universal links work: tapping `https://trace.app/m/sessions/...` opens the app
...
3. On a real device, SMS yourself `https://trace.app/m/sessions/g1/s1` → tap → app opens directly to session.
